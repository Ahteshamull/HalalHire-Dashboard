...
const AppSidebar = () => {
  ...
  return (
    <Sidebar 
      className="mt-0 ml-0 pb-7 md:mt-4 md:ml-4 md:mb-4 md:rounded-3xl border-none shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(255,255,255,0.03)] bg-background/95 backdrop-blur-3xl overflow-hidden relative z-50 transition-all duration-500 ease-in-out h-full" 
      collapsible="icon"
    >
      ...
      {/* sidebar header */}
      <SidebarHeader className="flex flex-col items-center py-6 md:py-8 relative z-10">
        <div className="flex w-full flex-col items-center gap-2 md:gap-3">
          <div className="relative mt-2 flex h-24 w-24 items-center justify-center group-data-[collapsible=icon]:mt-0 group-data-[collapsible=icon]:h-12 group-data-[collapsible=icon]:w-12 md:mt-4 md:h-28 md:w-28 transition-all duration-500">
      ...
      <SidebarSeparator className="mx-auto w-2/3 max-w-xs bg-gradient-to-r from-transparent via-border to-transparent opacity-50 relative z-10 mb-4" />

      <SidebarContent className="px-3 py-2 relative z-10 scrollbar-none">
        <SidebarGroup>
          <SidebarGroupContent>
              ...
              {items.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton
                      asChild
                      className="h-12 w-full p-0 md:h-14 bg-transparent hover:bg-transparent"
                      isActive={active}
                    >
                      <Link
                        href={item.url}
                        onClick={() => isMobile && setOpenMobile(false)}
                        className={`group relative flex h-full w-full items-center gap-4 overflow-hidden rounded-2xl px-5 transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0 ${
                          active
                      ...
                      >
                        <div className={`flex items-center justify-center p-2 rounded-xl transition-colors duration-300 ${active ? "bg-primary-foreground/20" : "bg-transparent group-hover:bg-background shadow-sm group-hover:shadow-md"}`}>
                          <item.icon
                            className={`h-5 w-5 transition-transform duration-300 group-data-[collapsible=icon]:h-6 group-data-[collapsible=icon]:w-6 ${
      ...
      {/* sidebar footer */}
      <SidebarFooter className="p-5 relative z-10 pb-8">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
